...
$app.directive('materialPickerInput', function() {
    return {
        restrict: 'A',
        controller: function($element) {
            this.$onInit = function() {
                let $input = $($element);
                let $wrapper = $input.parents('.colorpicker-wrapper');
                let $picker = $input.next('material-picker');

                $wrapper.find('.ui.label').on('click',function() {
                    $picker.addClass('active');
                });

                $(document).on('click',function($e) {
                    let $target = $($e.target);
                    let $parent = $target.parents('.colorpicker-wrapper');
                    if ($parent.length == 0) $picker.removeClass('active');
                });
            }
        }
    };
        ...
        return {
            ...
            link: function ($scope, element, attrs, ngModel) {
                ...
                var isHex = function (value) {
                    ...
                    var hex = (value.constructor === Object) ? value.hex : value;
                ...
                $scope[watch](attrs.ngModel, function (value, oldValue) {
                    ...
                    if (value.constructor === String) value = {hex:value};
